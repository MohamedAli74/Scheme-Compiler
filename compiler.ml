...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmLambda (params', Opt opt, expr) ->
        let params2 = params' @ [opt] in
        
        ScmLambda' (params', Opt opt, run expr params2 (params :: env))
      | ScmApplic (proc, args) ->
  ...
  let annotate_tail_calls = 
    let rec run in_tail = function
      ...
      | ScmApplic' (proc, args, app_kind) ->
          let apptype = if in_tail then Tail_Call else Non_Tail_Call
          in ScmApplic'(run false proc,List.map (run false) args,apptype)
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmApplic' (proc, args, Non_Tail_Call) -> 
         ...
         (List.map
         (fun arg ->
         let arg_code = run params env arg in (*run is the compiler's recursive main function*)
         arg_code
         ^ "\tpush rax\n")
         (List.rev args)) in
